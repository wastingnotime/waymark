...
def test_cancellation_cannot_be_scheduled_in_the_past():
    domain = subscribed()
...
def test_entry_body_must_be_text():
    domain = subscribed()
    with pytest.raises(DomainError, match="entry body must be text"):
...
def test_payment_success_after_cancellation_is_rejected():
    domain = subscribed()
    domain.cancel_subscription(instant(4), instant(2))
